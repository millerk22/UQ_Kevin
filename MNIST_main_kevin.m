...
fid = {}; 
for i = 1:NClass
    class_i = find(ground_truth == i);  % indices of ith class
    class_i = class_i(randperm(length(class_i)));  % permute these, then take the fidelity_percent of them
    fid{i} = class_i(1:ceil(fidelity_percent * length(class_i)));
end
disp(['Number of points: ', num2str(N)]);
disp(['Percent that are fidelity: ', num2str(fidelity_percent)]);
...
disp('Starting MCMC...');
tic;
gamma=0.1; % obs noise std, a hyperparameter specified
max_iter=2*10^4; % number of mcmc steps
%Phi_eval = get_multiclass_likelihood_function('bls', gamma);
%E(1) = 0.2; 
[m1, iter_stats1] = mcmc_gibbs_probit_multiclass_kevin(max_iter, gamma,  ...
                            V, E,fid); 
